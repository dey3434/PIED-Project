...
PIPO.nm <- NM.all %>% filter(SPCD %in% "122")
...
NM.meta.ll<- left_join( PIPO.nm,NM.PLOT.small, by = c("COUNTYCD", "STATECD", "PLOT"))

summary(NM.meta.ll %>% filter(!is.na(LON)))
PIPO.nm <- NM.meta.ll
PIPO.nm$dataset <- "New Mexico"

PIPO.nm.ll <- unique(PIPO.nm[,c("LAT", "LON", "CN", "dataset")])
colnames(PIPO.nm.ll)[3] <- "name"

# read in AZ data:
PIPO.az.meta <- read.delim("FIA_inc_data/PIPOCores518Meta.txt")
PIPO.az.ll <- unique(PIPO.az.meta[,c("LAT", "LON", "CORE_CN")])
PIPO.az.ll$dataset <- "Arizona"
colnames(PIPO.az.ll)[3] <- "name"

PIPO.ll <- rbind(PIPO.outside.ll, PIPO.nm.ll, PIPO.az.ll)
...
saveRDS(ppt.extracted, here("FIA_inc_data","pipo_extracted.ppt.data_v5.rds"))

ppt.extracted <- readRDS( here("FIA_inc_data","pipo_extracted.ppt.data_v5.rds"))
...
saveRDS(temp.extracted, here("FIA_inc_data","pipo_extracted.temp.data_v5.rds"))
temp.extracted <- readRDS( here("FIA_inc_data","pipo_extracted.temp.data_v5.rds"))
...
saveRDS(temp.max.extracted, here("FIA_inc_data","pipo_extracted.temp.max.data_v5.rds"))
temp.max.extracted <- readRDS( here("FIA_inc_data","pipo_extracted.temp.max.data_v5.rds"))
...
write.csv(full.ppt.tmean.3, here("FIA_inc_data","pipo_all_tmean_ppt_v5.csv"), row.names = FALSE)
