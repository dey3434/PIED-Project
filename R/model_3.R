...
csvfiles <- here::here("results", paste0("model_3_sizefix_", 1:3, ".csv"))

if (all(file.exists(csvfiles))) {
  message("loading saved MCMC run")
  fit_grow <- read_stan_csv(csvfiles, col_major = TRUE)
} else {
  message("Starting new MCMC run")
  fit_grow <- stan(file = 'stancode/model_3.stan', data = pied_dat,
                   iter = 5000,
                   warmup = 1000,
                   chains = 3, cores = 8,
