...
grow.new$decade <- floor(grow.new$year / 10) * 10
grow.monsoon.10<-na.omit(grow.new) %>%
  group_by(decade, name, PLOT, SUBP) %>%
  summarise(growth_ten_year = sum(growth),
            n_years = n(),
            tmp_10_norm = mean(tmp_norm),
            ppt_10_norm = mean(ppt_norm),
            DIA_10_prev = mean(DIA_prev)) %>% ungroup() %>%
            filter(decade %in% c("1970", "1980"))




#grow.monsoon<-na.omit(grow.monsoon.10) %>% 
#filter(n_years == 10) %>%
#group_by(name) %>%
#filter(length(decade) > 1) %>%
#ungroup() %>%
#arrange(PLOT,SUBP,name) %>%
#mutate(PlotCD=as.numeric(factor(PLOT, levels = unique(PLOT))),
#treeCD=as.numeric(factor(name,levels=unique(name))),
#growth2=ifelse(growth_ten_year==0,0.001,growth_ten_year),loggrowth=log(growth2)) %>%
#group_by(PlotCD)%>%
#mutate_at(scale, .vars = vars(Precip_10_JulAug, Precip_10_NovDecJanFebMar, Tmean_10_AprMayJun, Tmean_10_SepOct)) %>%
#na.omit() %>%
#ungroup()
...
grow.monsoon<-na.omit(grow.monsoon.10) %>% 
  ...
  group_by(name) %>%
  filter(length(decade) > 1) %>%
  ungroup() %>% mutate_at(scale, .vars = vars(ppt_10_norm, tmp_10_norm)) %>% 
  arrange(PLOT,SUBP,name) %>%
...
xG<-as.matrix(cbind(grow_train$ppt_10_norm, grow_train$tmp_10_norm, grow_train$DIA_10_prev,
                    grow_train$ppt_10_norm*grow_train$tmp_10_norm,
                    grow_train$ppt_10_norm*grow_train$DIA_10_prev,
                    grow_train$tmp_10_norm*grow_train$DIA_10_prev))
xGtest<-as.matrix(cbind(grow_test$ppt_10_norm, grow_test$tmp_10_norm, grow_test$DIA_10_prev, 
                        grow_test$ppt_10_norm*grow_test$tmp_10_norm,
                        grow_test$ppt_10_norm*grow_test$DIA_10_prev,
                        grow_test$tmp_10_norm*grow_test$DIA_10_prev))
...
saveRDS(fit_grow, file = "20 yr emily retest.RDS")
summary<-summary(fit_grow)
summary

plotdata<-select(as.data.frame(fit_grow),"yrep[1]":"yrep[573]")
plotdatainterval<-select(as.data.frame(fit_grow), "u_beta[1]":"u_beta[6]")
colnames(plotdatainterval) <- c("u_beta_ppt_norm", "u_beta_tmp_norm", "u_beta_DIA_prev",
                                "u_beta_ppt_norm_tmp_norm",
                                "u_beta_ppt_norm_DIA_prev",
                                "u_beta_tmp_norm_DIA_prev")
