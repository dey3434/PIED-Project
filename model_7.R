...
csvfiles <- here::here("results", paste0("model_7_", 1:3, ".csv"))
...
if (all(file.exists(csvfiles))) {
  ...
  fit_grow <- stan(file = 'model_7.stan', data = pied_dat, 
                   ...
                   warmup = 1000,
                   chains = 3, cores = 8, 
                   sample_file = here::here("results", "model_7"))
}

# chain1 <- rstan::read_stan_csv("/home/rstudio/model_7_1.csv")
# chain2 <- rstan::read_stan_csv("/home/rstudio/model_7_2.csv")
# chain3 <- rstan::read_stan_csv("/home/rstudio/model_7_3.csv")

# fit_grow <- rstan::read_stan_csv(csvfiles)

# not sure this is the right name for the model -- IT WAS NOT!
# saveRDS(fit_grow, file = "model_7.RDS")
...
write.csv(beta.summaries, "betasummaries_model7_threechain_PIED.csv", row.names = FALSE)
...
##### 
save(p.o.df, file = here::here("results", "model-7-pred-obs.RData"))
...
p_pred_vs_observed <- ggplot(p.o.df, aes(predicted, observed)) + 
  ...
  ylim(0, 10) + xlim(0,10)
p_pred_vs_observed
ggsave(here::here("images", "model_7", "pred_vs_observed.png"), p_pred_vs_observed)
...
save(ll, r_eff, leaveoneout, file = here::here("results", "model-7-loo.RData"))
...
ggsave(here::here("images", "model_7", "ppc-plot-1.png"),
       ppc1, width = 16/3, height = 9)
ggsave(here::here("images", "model_7", "ppc-plot-2.png"),
       ppc2, width = 16/3, height = 9)
ggsave(here::here("images", "model_7", "ppc-plot-3.png"),
...
# newll <- as.matrix(ll)
# r_eff <- relative_eff(exp(ll), chain_id = rep(1:3, each = 4000), cores = 1)
# leaveoneout <- loo(as.matrix(ll), r_eff = r_eff, save_psis = TRUE, cores = 1)
# 
# yrep <- matrix(0, length(sigma), length(yG))
# for(i in 1:length(sigma)){
